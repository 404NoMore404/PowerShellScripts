...
<#
Insert your Connectto-MGGraph function here
Make sure it connects using:
Connect-MgGraph -Environment USGovDoD -Scopes "DeviceManagementManagedDevices.Read.All","Directory.Read.All"
#>

# ================================
# Get Policies and Groups for a Device (DoD)
# ================================
function Get-DeviceAssignments {
    param(
        [Parameter(Mandatory)]
    ...
    # Get configuration policies assigned to this device (DoD Graph)
    $policies = Invoke-MgGraphRequest `
        -Method GET `
        -Uri "https://dod-graph.microsoft.us/beta/deviceManagement/managedDevices/$DeviceId/deviceConfigurationStates" `
        | Select-Object -ExpandProperty value

    # Get Azure AD Groups the device is a member of (DoD Graph)
    $groups = Invoke-MgGraphRequest `
        -Method GET `
        -Uri "https://dod-graph.microsoft.us/v1.0/devices/$DeviceId/memberOf" `
        | Select-Object -ExpandProperty value
...
function Start-DeviceComparisonMenu {
    ...
    # Ensure Connectto-MGGraph or equivalent DoD connection function is called here
    Connectto-MGGraph
