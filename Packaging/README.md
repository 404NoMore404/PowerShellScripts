...
| `Watch-Install.ps1` | Pre-packaging observation tool — run on a test machine before packaging |
...
Before writing a single configuration value, run the installer through Watch-Install.ps1 with the `-WatchRegistry` and `-WatchFiles` flags. This tells you exactly what the installer writes — the DisplayName, DisplayVersion, registry key location, and any folders created. These are the values you will paste into your configuration blocks.
...
The `PSChildName` column is the value you need. Alternatively, run Watch-Install.ps1 with `-WatchRegistry` and it will report this for you automatically.
...
Watch-Install.ps1 is a pre-packaging observation tool. Run it on a clean test machine or snapshot VM before filling in any configuration values. It runs the installer, captures everything that changes, and prints a structured report you can read directly into your configuration blocks.
...
When run with `-WatchRegistry`, it takes a before snapshot of all Uninstall registry hives, runs the installer, takes an after snapshot, and prints every new key that appeared. For each new key it prints the DisplayName and DisplayVersion with a prompt showing exactly what to paste into `$DisplayNameFilter` in both Detect-APPNAME.ps1 and Invoke-IntuneInstall.ps1.

When run with `-WatchFiles`, it snapshots top-level directories in Program Files, LocalAppData, and AppData before and after, and reports any new folders created.

It also captures stdout and stderr from the installer process, translates the exit code to a human-readable description, and saves a full log to the IME logs folder.

**Usage:**

```powershell
# Observe registry and filesystem changes
.\Watch-Install.ps1 -InstallerPath ".\setup.exe" -Arguments "/VERYSILENT" -WatchRegistry -WatchFiles

# Registry only (faster for MSI packages)
.\Watch-Install.ps1 -InstallerPath ".\installer.msi" -Arguments "/qn" -WatchRegistry
```
...
|-----------|------------|
| `-InstallerPath` | Path to the installer EXE or MSI |
| `-Arguments` | Silent install arguments to pass to the installer |
| `-LogPath` | Log output path. Defaults to IME logs folder with timestamp. |
| `-WatchRegistry` | Enable registry before/after snapshot |
| `-WatchFiles` | Enable filesystem before/after snapshot |

**Reading the output for detection configuration:**

The registry diff output includes a line like:

```
[SUCCESS]   DisplayName    : 7-Zip 24.08  <-- use as: "*7-Zip 24.08*"
[SUCCESS]   DisplayVersion : 24.08.0.0
```

The suggested filter on the DisplayName line is ready to paste directly into `$DisplayNameFilter` in Detect-APPNAME.ps1 and `$DetectionNameFilter` in Invoke-IntuneInstall.ps1.

If no new registry keys appear in the diff, the app does not register in the standard Uninstall hives. Switch to File detection and use a new directory reported by `-WatchFiles` as your detection path.
...
If using the DisplayName method, the wildcard filter may not match the app's actual display name. Run Watch-Install.ps1 with `-WatchRegistry` after a manual install — it prints the exact DisplayName with a ready-to-paste filter suggestion. If using RegistryGUID, the path or version string may not match exactly.
...
The installer does not write to the standard Uninstall hives. This means Registry and DisplayName detection methods will not work for this app. Use File detection instead, with a path from the `-WatchFiles` output.
