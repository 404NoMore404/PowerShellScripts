...
**Step 3c — License configuration.** If the app requires a license to function, configure it here. Choose from four options:

- **None** — no licensing step needed (default). Skips this section entirely.
- **File** — a license file (`.lic`, `.dat`, `.xml`, etc.) that must be copied to a specific path on the target machine. A file picker opens so you select the license file, then you choose the destination path. The file is bundled into the Source folder and the generated install script copies it to the destination after the installer exits.
- **Server** — a license server address (e.g. `27000@licserver.company.com`) that must be written to a registry key. You provide the server address and the full registry key path. The generated install script writes the value post-install.
- **Key** — a post-install activation key run via command line. You provide the key and the activation command with `{KEY}` as a placeholder. A preview of the resolved command is shown before confirming. The activation runs automatically after the installer exits and the exit code is logged.
...
**Step 7 — Build package.** Intunator creates the output folder, copies the installer (and license file if configured), generates all three scripts, and writes Package-Summary.txt. If IntuneWinAppUtil.exe is found on the machine (or you browse to it), it optionally wraps everything into a `.intunewin` file and opens the folder in Explorer.
...
[AppName]_[Version]\
  Source\
    [installer file]
    [license file]              (only if license type is File)
    Install-[AppName].ps1
...
Open `Package-Summary.txt`. Every Intune upload field is pre-filled: app name, publisher, version, install command, uninstall command, detection method, requirements, licensing configuration, and step-by-step upload instructions.
...
Intunator is the all-in-one guided packager. Run it on your packaging machine and it handles installer detection, silent argument verification, registry-based uninstall discovery, license configuration, detection rule configuration, and output file generation in a single session.

**What it generates:**

| File | Purpose |
|------|---------|
| `Install-[AppName].ps1` | Intune-ready silent install wrapper with logging and optional license deployment |
...
| `Package-Summary.txt` | Every Intune upload field pre-filled, plus licensing configuration and step-by-step upload instructions |
...
| Squirrel | `--silent` |
| Unknown EXE | `-s` tried first, then `/s`, `--silent`, `/silent`, `/quiet`, `/S`, and others (medium confidence — escalation flag added) |
...
Intunator supports three types of post-install license deployment. All are configured in Step 3c and injected into the generated install script as a block that runs after the installer exits.

| License type | What it does |
|-------------|-------------|
| File | Copies a bundled license file from the package Source folder to a specified destination path on the target machine. Creates the destination directory if it does not exist. |
| Server | Writes a license server address to a specified registry key. Useful for FlexLM, RLM, and similar floating license systems. |
| Key | Runs a post-install activation command with the license key substituted in. Logs the activation exit code and warns if non-zero. |

If a license file is configured, it is automatically bundled into the Source folder alongside the installer so it travels with the package. The Package-Summary.txt LICENSING section describes exactly what was configured, with the activation key value hidden so the summary is safe to share.
...
Any condition Intunator cannot fully resolve generates an escalation flag. Common triggers include: medium-confidence installer type, install args not verified by a live test, Squirrel-style user-profile install, app not found in registry on the packaging machine, uninstall path inside a user profile, license file or server not fully configured, and RegistryGUID detection selected. All flags appear in Escalation-Notes.txt with a specific explanation and suggested resolution for each.
...
**IntuneWinAppUtil.exe search paths:**

Intunator searches for IntuneWinAppUtil.exe in the following locations, in order:

1. Same folder as `Intunator.ps1`
2. Current working directory
3. `C:\Tools\IntuneWinAppUtil.exe`
4. `[UserProfile]\Downloads\IntuneWinAppUtil.exe`

If not found in any of these locations, Intunator prompts you to browse for it or skip wrapping. Download it from: `https://github.com/microsoft/Microsoft-Win32-Content-Prep-Tool`
...
The `PSChildName` column is the value you need. If you ran Watch-Install first, this is already printed in the registry section of the report — no separate lookup needed.
...
At Step 3c, if Chrome requires no licensing, press Enter to accept None and move on. If you are packaging a licensed app, select the appropriate type and follow the prompts.
...
|----------------|--------|---------|
| `[APPNAME]\[APPNAME]_Installer_YYYY-MM-DD_HHMMSS.log` | Intunator-generated Install script | Full install log including license deployment |
...
.\Install-AppName.ps1
# Verify install and license deployment, then:
.\Uninstall-AppName.ps1
# Verify removal, then:
...
**License file is missing from the package after build**

Intunator copies the license file into the Source folder during Step 7. If the file is missing, the most likely cause is that the source path was no longer accessible when the build ran (mapped drive disconnected, file moved, etc.). Re-run Intunator and re-select the license file in Step 3c, or manually copy the file into the Source folder and verify the destination path in the install script matches where the app expects it.

**License server setting is not being applied on target devices**

Check that the registry key path entered in Step 3c is correct for the version of the app being deployed. Some vendors change the registry path between major versions. Install the app on a test machine, configure the license server manually through the app's own interface, then use RegEdit or Watch-Install to find the exact key and value the app wrote. Update the path in the install script accordingly.

**License activation command returns a non-zero exit code**

A non-zero exit code from the activation command is logged as a WARN rather than an ERROR, so the install script will not fail. Check the install log for the activation exit code line and look up the vendor's documentation for that specific code. Common causes are the key already being activated on another machine, the key format being wrong for the installed version, or the activation executable requiring network access that is blocked by firewall policy.
...
If you are using the DisplayName method, the wildcard filter probably does not match the app's actual display name. Run Watch-Install and check the DisplayName line in the registry section — it prints a ready-to-paste suggestion right there in the output. If you used Intunator, check that the DisplayName filter in `Detect-[AppName].ps1` matches what appears in Apps and Features after a real install.
...
| 2.5 | 03/13/2026 | Watch-Install.ps1 updated to v1.6 (ExitMeaning / duplicate JSON block fixes). Watch-Uninstall.ps1 updated to v1.1 (field-path fixes for v1.6 schema). README fully rewritten. |
...
| 2.7 | 03/13/2026 | Intunator.ps1 updated. Added Step 3c — License Configuration: supports File (bundled .lic/.dat/etc. copied to target path), Server (license server address written to registry key), and Key (post-install activation command with key substitution) license types. License file is automatically bundled into the Source folder at build time. Package-Summary.txt now includes a LICENSING section. IntuneWinAppUtil.exe search extended to include the user's Downloads folder. Silent arg auto-try loop fixed — no longer skips remaining combinations after a failed silence check. Default unknown EXE arg order updated to lead with -s and /s before /S. |# Intune Win32 Package Templates
